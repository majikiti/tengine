...
class System: Loggable {
  ...
  void loop() {
    ...
    if(ctx.im.keyOnce(cf.wakeupDebugKey)) ctx.debugging = !ctx.debugging;
    ...
    // background
    SDL_SetRenderDrawColor(ctx.r, cf.bgColor.r, cf.bgColor.g, cf.bgColor.b, cf.bgColor.a);
...
auto objectPickup(bool trigger, bool outside){
    ...
    foreach(i, p; gos){
      ...
      flag &= outside ? tform.hidein(col.size, 300) : tform.isin(col.size);
      if(flag) res ~= p;
    }
    return res;
}
